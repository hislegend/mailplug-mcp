...
- Microsoft 365 커넥터로 우회해도 회사 메일을 Microsoft Cloud로 동기화해야 하고, 발신은 valla@crabs.ai 명의로 못 보냅니다.
...
git clone https://github.com/crabs-ai/mailplug-mcp.git
cd mailplug-mcp
uv venv
...
```env
MAILPLUG_EMAIL=valla@crabs.ai
MAILPLUG_DISPLAY_NAME=이종권
MAILPLUG_SIGNATURE_MODE=auto
MAILPLUG_SIGNATURE_NAME=이종권
MAILPLUG_SIGNATURE_TITLE=대표
MAILPLUG_SIGNATURE_COMPANY=주식회사 크랩스
MAILPLUG_SIGNATURE_PHONE=010-0000-0000
MAILPLUG_SIGNATURE_WEBSITE=crabs.ai
```
...
```bash
mailplug-mcp-setup
# Mail Plug 이메일 주소 [valla@crabs.ai]: ↵
# 앱 비밀번호: (입력 시 표시되지 않음)
# ✓ valla@crabs.ai 의 앱 비밀번호가 키체인에 저장되었습니다.
```
...
삭제: `mailplug-mcp-setup --reset --email valla@crabs.ai`.
...
> 사용자: "valla@crabs.ai 발신으로 imnotcut@gmail.com 한테 '주간 보고 첨부 드립니다' 라는 메일 써줘. 본문은 이번 주 핵심 3가지 정리해서 한 줄씩, 마지막에 다음 주 일정 두 줄."
